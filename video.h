...
C_RESULT video_codec_open( video_controller_t* controller, codec_type_t codec_type )
{
  ...
  controller->cache_mbs = (video_macroblock_t*)malloc( 2 * MAX_NUM_MACRO_BLOCKS_PER_CALL * sizeof(video_macroblock_t) );
...
{
  vp_api_picture_t blockline = { PIX_FMT_YUV420P };
...
C_RESULT video_packetizer_init( video_controller_t* controller )
{
  ...
  controller->in_stream.bytes     = (quint32*)malloc( DEFAULT_INTERNAL_STREAM_SIZE );
  controller->in_stream.used      = 0;
...
//  ----------------------- <= ------------ <= stream
//         code                stream->bits
C_RESULT inline video_read_data( video_stream_t* const stream, uint32_t* code, int32_t length )
{
...
C_RESULT inline video_peek_data( const video_stream_t* const stream, uint32_t* code, uint32_t length )
{
  uint32_t out_code = *code;
...
C_RESULT inline video_align8( video_stream_t* const stream )
{
...
C_RESULT uvlc_decode( video_stream_t* const stream, int32_t* run, int32_t* level, int32_t* last)
{
  ...
  video_peek_data( stream, &stream_code, (uint32_t)32 );
...
static C_RESULT uvlc_load_stream( video_stream_t* out, video_stream_t* in )
{
  ...
  {
    out->bytes = (quint32*)realloc( out->bytes, out->size + 2048 ); // Add 2KB to internal stream
    out->size += 2048;
  }
...
//                (controller->num_blockline > 0 && controller->blockline < controller->num_blockline) );
  if( !(controller->blockline == 0x1F ||
      controller->num_blockline == 0 || // Check if cache is allocated for current picture
      (controller->num_blockline > 0 && controller->blockline < controller->num_blockline)))
      qDebug(" controller->blockline == 0x1F || controller->num_blockline == 0 || // Check if cache is allocated for current picture (controller->num_blockline > 0 && controller->blockline < controller->num_blockline)");
...
C_RESULT uvlc_encode_blockline( video_controller_t* controller, const vp_api_picture_t* blockline, bool_t picture_complete )
{
  ...
  {
    stream->bytes = (quint32*)realloc( stream->bytes, stream->size + 2048 ); // Add 2KB to internal stream
    stream->size += 2048;
  }
...
//  if(!((width != 0) && (height != 0)))
//     qDebug("(width != 0) && (height != 0) width:%d heigth: %d",width,height);
...
inline C_RESULT do_unquantize(int16_t* ptr, int32_t picture_type, int32_t quant, int32_t num_coeff)
{
  int32_t coeff;
